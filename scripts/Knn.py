...
distance = math.sqrt(
    (pointNew[0] - pointOld[0]) ** 2 + (pointNew[1] - pointOld[1]) ** 2
)
...
def calculate_distances(target_point, df):
    # Find distances from target to all labeled points
    ...
    for _, row in categorized.iterrows():
        ...
        dist = math.sqrt(
            (target[0] - other_point[0]) ** 2 + (target[1] - other_point[1]) ** 2
        )
...
def final_decision(top3):
    # Majority vote among the 3 nearest neighbors
